...
docker buildx build --platform "linux/amd64,linux/arm64,linux/arm/v7" --build-arg NGINX_VERSION=$NGINX_VERSION --build-arg PHP_LATEST_VERSION=$PHP_LATEST_VERSION --build-arg PHP_STABLE_VERSION=$PHP_STABLE_VERSION --build-arg PHP_REDIS_VERSION=$PHP_REDIS_VERSION --no-cache -t registry.cn-hangzhou.aliyuncs.com/nuoyis-lnmp-np:latest -t registry.cn-hangzhou.aliyuncs.com/nuoyis-lnmp-np:$build_version -f dockerfile/dockerfile --push .
